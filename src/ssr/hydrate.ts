...
export const hydrateEider = (
  ...
  Effect.gen(function* () {
    // createEiderApp with ssr:false uses createSSRApp for hydration
    // Vue's hydration kicks in when the DOM already has server-rendered HTML
    const app = yield* createEiderApp({ ...input, ssr: false })
    yield* Effect.try({
      try: () => app.mount(selector),
      catch: (e) =>
        new RuntimeError({ message: `Hydration failed: ${String(e)}`, cause: e }),
    })
  })
